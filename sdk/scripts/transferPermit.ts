...
import { permit3ABI, solmateMockErc20ABI } from "../src/generated";
import {
  permit3SignTransfer,
  permit3TransferBySignature,
} from "../src/permit3";
import { ALICE, BOB, forkBlockNumber, forkUrl } from "../src/test/constants";
import { anvil, publicClient, walletClient } from "../src/test/utils";
...
const main = async () => {
  ...
  const transfer = {
    transferDetails: makeCurrencyAmountFromString(mockERC20, "1"),
    spender: ALICE,
    nonce: 0n,
  ...
  const { hash } = await permit3TransferBySignature(
    ...
    ALICE,
    {
      ...
      requestedTransfer: {
        to: BOB,
        amount: makeCurrencyAmountFromString(mockERC20, "0.5"),
      },
      signature,
    },
  );
